...
def do_that_func_if_css_element_is_located_with(orig_func):
...
@do_that_func_if_css_element_is_located_with
...
@do_that_func_if_css_element_is_located_with
def click_button(css_class_name: str, with_driver: webdriver):
...
with WebDriver(webdriver.Chrome('./chromedriver')) as driver:
    ...
    send_to(css_class_name='js-signin-login', with_driver=driver, keys=secret_keys.badoo_email)
    send_to(css_class_name='js-signin-password', with_driver=driver, keys=secret_keys.badoo_pass)
    click_button(css_class_name='sign-form__submit', with_driver=driver)
    time.sleep(2)
